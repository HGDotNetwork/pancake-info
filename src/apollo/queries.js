...
export const ALL_PAIRS = (block) => {
  ...
  query pairs($skip: Int!) {
    pairs(first: 100, skip: $skip, orderBy: trackedReserveBNB, orderDirection: desc, where: { createdAtBlockNumber_gte: 4519965 }) {
      id
      token0 {
        id
        symbol
...
export const PAIRS_CURRENT = gql`
  query pairs {
    pairs(first: 100, where: { createdAtBlockNumber_gte : 4519965 },  orderBy: trackedReserveBNB, orderDirection: desc) {
      id
    }
  }
`
...
export const PAIRS_HISTORICAL_BULK = (block, pairs) => {
  ...
  let queryString = `
  query pairs {
    pairs(first: 200, where: {id_in: ${pairsString}}, orderBy: trackedReserveBNB, orderDirection: desc) {
      id
      reserveUSD
      trackedReserveBNB
      volumeUSD
